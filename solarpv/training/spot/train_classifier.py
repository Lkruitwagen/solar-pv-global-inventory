...
params = {
    ...
    'training_kwargs': {
        ...
        'epochs': 2,
        'initial_epoch': 0,
        'optimizer': LRMultiplierAdam(multipliers={'dense_1': 10.0},
                                      lr=1e-4),
...
def train(params=params,
    ...
    # Set output files
    model_fname = os.path.join(params['global_metadata']['local_model'], params['model_name']+'.hdf5')
    history_fname = os.path.join(params['global_metadata']['local_model'], params['model_name']+'_history.pkl')
    tensorboard_dir = os.path.join(params['global_metadata']['local_model'], 'tb')
    loss_csv = os.path.join(params['global_metadata']['local_model'], params['model_name']+'_loss.csv')
    ...
    model.compile(optimizer=sgd,
                  loss=params['training_kwargs']['loss'],
                  metrics=['accuracy'])
    ...
    print(ptrain['batchsize_train'], len(train_generator), ptrain['batchsize_val'], len(validation_generator))
    ...
    cb_checkpointer = ModelCheckpoint(filepath=model_fname,
    ...
    callbacks_list = [cb_checkpointer, cb_early_stopper, csv_logger, lr_callback, tensorboard]
    ...
    # Train the model!
    fit_steps = ptrain['steps_per_epoch'] if 'steps_per_epoch' in ptrain else n_train_batches
    val_steps = ptrain['validation_steps'] if 'validation_steps' in ptrain else n_val_batches
