...
def binary_crossentropy(y_true, y_pred, small=1e-3, weight=1.0, sample_weight=None):
    ...
    bce = weight * y_true * tf.math.log(y_pred + small) \
        + (1 - y_true) * tf.math.log(1 - y_pred + small)
...
def train_from_document(params=params):
    ...
    if not os.path.exists(data_path):
        print("working dir {} doesn't exist. You need to get your training data!".format(data_path))
    ...
    # Output file names
    model_fname, checkpoint_stub, loss_fname = return_model_file_paths(local_output_dir, model_name)
    ...
    assert_consistency(model, shape=(kw_train['batchsize'],
                                     tilesize, tilesize, len(params['data_metadata']['bands'])))
    ...
    # ---------------------------
    train_keys_file = os.path.join(params['global_metadata']['local_ground'], kw_train['datalist'])
    val_keys_file = os.path.join(params['global_metadata']['local_ground'], kw_train['val_datalist'])
    
    trn_generator = DataGenerator(train_keys_file,
    ...
    val_generator = DataGenerator(val_keys_file,
    ...
    if (n_train == 0) or (n_val == 0):
        raise ValueError("Missing train/validation data: ",
                         "n_train = %d, n_val = %d. path = %s." %
                         (n_train, n_val, data_path))
    ...
    checkpoint = ModelCheckpoint(checkpoint_fname,
                                 monitor='val_loss',
                                 verbose=0,
                                 save_best_only=True,
    ...
    lr_callback = ReduceLROnPlateau(monitor='val_loss', factor=0.2, patience=10, min_lr=1e-7, verbose=1)
    ...
    early_stopping = EarlyStopping(monitor='val_loss', min_delta=1e-4, patience=15, verbose=1)
    ...
    if kw_train['batchsize'] > n_train:
        warnings.warn("batch size {} bigger than training set {}. "
                      "Setting batchsize=n_train.".format(kw_train['batchsize'], n_train))
        kw_train['batchsize'] = n_train
    if kw_train['val_batchsize'] > n_val:
        warnings.warn("batch size {} bigger than val set {}. "
                      "Setting batchsize=n_val.".format(kw_train['val_batchsize'], n_val))
        kw_train['val_batchsize'] = n_val
    
    fit_steps = kw_train['steps_per_epoch'] if 'steps_per_epoch' in kw_train else n_train//kw_train['batchsize']
    val_steps = kw_train['validation_steps'] if 'validation_steps' in kw_train else n_val//kw_train['batchsize']
