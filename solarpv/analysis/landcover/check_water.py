...
def poly2staticimg(poly_wgs, idx):
    ...
    fig.savefig(os.path.join(os.getcwd(),'data','check_water',f'{idx}.png'))
    plt.close()


def get_water_samples(gdf_path):
	gdf = pd.DataFrame(gpd.read_file(gdf_path))
	print (gdf['lc_vis'].unique())
	print ((gdf['lc_vis']=='other').sum())
	gdf[gdf['lc_vis']=='other'].progress_apply(lambda row: poly2staticimg(row['geometry'], row['unique_id']), axis=1)


if __name__=="__main__":
	get_water_samples(os.path.join(os.getcwd(),'data','SPV_vis.gpkg'))