...
class Landmark:
    ...
    def get_batches(self,json_dir,done_dir, start_idx, end_idx, batch=1):
        ...
        done_idxs = [os.path.split(f)[-1].split('.')[0]  for f in glob.glob(os.path.join(done_dir,'*.csv'))]
    ...
    def __load_driver(self):
        ...
        WebDriverWait(self.driver,10).until(lambda d: d.find_element_by_tag_name('div.close-icon.pointer'))
    ...
    def _get_batch(self, batch):
        ...
        for rec in batch:
            ...
            fname = os.path.join(os.getcwd(),'data','landmark_results',f'{rec["idx"]}.csv')
...
if __name__ == '__main__':
    ...
    json_dir = os.path.join(os.getcwd(),'data','landmark')
    done_dir = os.path.join(os.getcwd(),'data','landmark_results')
